---
id: "post_9"
title: "Binalarda Enerji Performansı (EKB) ve NZEB Yönetmeliğinin İmar Süreçlerine Etkisi"
...
<p>Projelerde EKB sınıfını belirleyen temel birim <strong>kWh/m²-yıl</strong> (yıllık metrekare başına düşen birincil enerji tüketimi) değeridir. Bu değer, elektrik veya mekanik projelerden tek başına hesaplanamaz; disiplinler arası entegre bir çalışmanın ürünüdür:</p>
<ul>
    <li><strong>Elektrik Projesinin Rolü:</strong> Tasarlanan aydınlatma armatürlerinin güç yoğunluğu (W/m² olarak) ve varsa güneş paneli kurulu güç verileri elektrik projelerinden çekilir.</li>
...
<table style="width: 100%; font-size: 0.8rem; border-collapse: collapse; text-align: left;">
    <thead>
        <tr style="border-bottom: 2px solid var(--border-color); color: var(--text-primary);">
            <th style="padding: 0.5rem 0;">Bina Tipi</th>
            <th style="padding: 0.5rem 0;">Enerji Sınıfı</th>
            <th style="padding: 0.5rem 0;">Tüketim Aralığı (kWh/m²-yıl)</th>
            <th style="padding: 0.5rem 0;">Tipik Özellikler</th>
        </tr>
    </thead>
    <tbody>
        <tr style="border-bottom: 1px solid var(--border-color); color: var(--text-secondary);">
            <td style="padding: 0.5rem 0; font-weight: 600;">Konut</td>
            <td style="padding: 0.5rem 0; color: #27ae60; font-weight: 700;">B Sınıfı (NZEB)</td>
            <td style="padding: 0.5rem 0;">50 - 89</td>
            <td style="padding: 0.5rem 0;">Kalın yalıtım, Low-E camlar, Isı pompası, LED, Güneş Paneli</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-color); color: var(--text-secondary);">
            <td style="padding: 0.5rem 0; font-weight: 600;">Konut</td>
            <td style="padding: 0.5rem 0; color: #f1c40f; font-weight: 700;">C Sınıfı (Yeni Bina)</td>
            <td style="padding: 0.5rem 0;">90 - 139</td>
            <td style="padding: 0.5rem 0;">Yönetmeliğe uygun mantolama, çift cam, merkezi pay ölçer doğalgaz</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-color); color: var(--text-secondary);">
            <td style="padding: 0.5rem 0; font-weight: 600;">Konut</td>
            <td style="padding: 0.5rem 0; color: #e67e22; font-weight: 700;">D - E Sınıfı</td>
            <td style="padding: 0.5rem 0;">140 - 220</td>
            <td style="padding: 0.5rem 0;">2000-2015 arası yapılar, zayıf/standart yalıtım, kombi ısıtma</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-color); color: var(--text-secondary);">
            <td style="padding: 0.5rem 0; font-weight: 600;">Ofis (Ticari)</td>
            <td style="padding: 0.5rem 0; color: #27ae60; font-weight: 700;">B Sınıfı</td>
            <td style="padding: 0.5rem 0;">80 - 149</td>
            <td style="padding: 0.5rem 0;">Yüksek havalandırma verimi, VRF iklimlendirme, LED, otomasyon</td>
        </tr>
        <tr style="border-bottom: 1px solid var(--border-color); color: var(--text-secondary);">
            <td style="padding: 0.5rem 0; font-weight: 600;">Ofis (Ticari)</td>
            <td style="padding: 0.5rem 0; color: #f1c40f; font-weight: 700;">C Sınıfı</td>
            <td style="padding: 0.5rem 0;">150 - 220</td>
            <td style="padding: 0.5rem 0;">Standart split klima, mekanik havalandırma, ortalama aydınlatma</td>
        </tr>
    </tbody>
</table>
</div>
...
<div>
<label style="font-size: 0.8rem; color: var(--text-muted); display: block; margin-bottom: 0.25rem;">Toplam İnşaat Alanı (m²)</label>
<input type="number" id="bepArea" value="3500" style="width: 100%; padding: 0.5rem; background: var(--bg-secondary); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: 4px;">
</div>
</div>
...
<input type="number" id="bepRenewable" value="8" style="width: 100%; padding: 0.5rem; background: var(--bg-secondary); border: 1px solid var(--border-color); color: var(--text-primary); border-radius: 4px;">
</div>
</div>
...
<span style="color: var(--text-secondary);">Hesaplanan Enerji Sınıfı (EKB):</span>
<strong id="bepClass" style="color: var(--accent-color);">-</strong>
</div>
<div style="display: flex; justify-content: space-between;">
<span style="color: var(--text-secondary);">NZEB (NSEB) Kapsam Durumu:</span>
...
<span style="color: var(--text-secondary);">Gerekçe / Notlar:</span>
<span id="bepNotes" style="color: var(--text-muted); font-size: 0.8rem; line-height: 1.2;">-</span>
</div>
</div>

<script>
function toggleRenewableInput() {
    var status = document.getElementById('bepStatus').value;
    var container = document.getElementById('renewableContainer');
    if (status === 'existing') {
...
function checkBepCompliance() {
    var status = document.getElementById('bepStatus').value;
    var area = parseFloat(document.getElementById('bepArea').value) || 0;
    ...
    document.getElementById('bepClass').innerText = energyClass + ' Sınıfı';
    document.getElementById('bepClass').style.color = classColor;
    document.getElementById('bepNzebStatus').innerText = nzebApplies ? 'KAPSAMDA (Limit 2000m²)' : 'Kapsam Dışı';
    
    var permitApproved = true;
    var notes = '';
    
    if (status === 'new') {
        if (nzebApplies) {
            // Must be at least B and renew >= 10%
    ...
    var resultEl = document.getElementById('bepPermitResult');
...
<h4 style="color: var(--text-primary); margin-top: 2rem; margin-bottom: 1rem;">3. İmar ve Ruhsat Sürecinde Enerji Tasarımı Adımları</h4>
<p>Mimari projelerin ruhsatlandırma (belediye onay) aşamasında izlenecek yol haritası şöyledir:</p>
<ol>
    <li><strong>Ön Tasarım ve Simülasyon:</strong> Mimar ve mekanik mühendisi, binanın cephe yalıtım kalınlıklarını, cam kombinasyonlarını ve mekanik tesisat verimliliklerini henüz avan proje aşamasındayken BEP-TR yazılımı standartlarına göre simüle etmelidir.</li>
    <li><strong>Yenilenebilir Enerji Entegrasyonu:</strong> 2.000 m² üzerindeki yapılarda güneş panelleri (PV) için çatı yerleşimi ve mekanik statik yük hesapları mimari/statik projeye işlenmeli, elektrik projesinde bağlantı şemaları gösterilmelidir.</li>
    <li><strong>EKB Düzenlenmesi ve Ruhsat Başvurusu:</strong> Projenin BEP-TR üzerinden onaylanmış EKB hesap raporu ruhsat dosyasına eklenir. Belediye imar müdürlüğü, EKB sınıfını ve yenilenebilir enerji yüzdesini denetledikten sonra yapı ruhsatı düzenler.</li>
    <li><strong>İskan (Yapı Kullanma İzin Belgesi) Aşaması:</strong> İnşaat bittiğinde, projede taahhüt edilen yalıtım ve güneş panellerinin sahada kurulduğu EKB uzmanı tarafından yerinde denetlenir. Projeye aykırı imalat yapılmışsa EKB onaylanmaz ve bina iskan alamaz.</li>
</ol>
